...
The `TourDialog` component is a multipart component. The styling needs to be
...
```jsx live=false
import { tourDialogAnatomy } from '@saas-ui/theme/anatomy'
import { createMultiStyleConfigHelpers } from '@chakra-ui/react'

const { definePartsStyle, defineMultiStyleConfig } =
  createMultiStyleConfigHelpers(tourDialogAnatomy.keys)
```
...
export const tourDialogTheme = defineMultiStyleConfig({ baseStyle })
```
...
import { tourDialogAnatomy } from './components/tour-dialog'

export const theme = extendTheme({
  components: { SuiTourDialog: tourDialogAnatomy },
})
```
...
export const tourDialogTheme = defineMultiStyleConfig({
  variants: { custom },
})

// Now we can use the new `custom` variant
<TourDialog variant="custom"> ... </TourDialog>
```
