...
The `StepForm` component is a multipart component. The styling needs to be
...
```jsx live=false
import { stepFormAnatomy } from '@saas-ui/theme/anatomy'
import { createMultiStyleConfigHelpers } from '@chakra-ui/react'

const { definePartsStyle, defineMultiStyleConfig } =
  createMultiStyleConfigHelpers(stepFormAnatomy.keys)
```
...
export const stepFormTheme = defineMultiStyleConfig({ baseStyle })
```
...
import { stepFormAnatomy } from './components/step-form'

export const theme = extendTheme({
  components: { SuiStepForm: stepFormAnatomy },
})
```
...
export const stepFormTheme = defineMultiStyleConfig({
  variants: { custom },
})

// Now we can use the new `custom` variant
<StepForm variant="custom"> ... </StepForm>
```
