...
```jsx live=false
import { tourAnatomy } from '@saas-ui/theme/anatomy'
...
export const tourTheme = defineMultiStyleConfig({ baseStyle })
```
...
import { tourAnatomy } from './components/tour'

export const theme = extendTheme({
  components: { SuiTour: tourAnatomy },
})
```
