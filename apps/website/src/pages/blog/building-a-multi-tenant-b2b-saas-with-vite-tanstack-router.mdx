...
<Info>
  This article was first written a year ago, and has been updated for the latest
  versions of the libraries, including Chakra UI v3 and Saas UI v3 (beta). The
  follow up articles are coming soon finally (I promise!).
</Info>
...
```bash
cd my-saas
npm i @tanstack/react-router @tanstack/react-router-devtools @tanstack/router-vite-plugin
```
...
export const Provider = () => {
  return (
    <SaasProvider linkComponent={LinkComponent} value={defaultSystem}>
      <RouterProvider router={router} />
    </SaasProvider>
  );
};
...
```tsx
import { forwardRef } from 'react'
import { LinkProps, SaasProvider, defaultSystem } from '@saas-ui/react'
import { RouterProvider, createRouter, Link } from '@tanstack/react-router'
...
import { useForm, FormLayout, SubmitButton } from '@saas-ui/forms'
  ...
  return (
    <Center height="$100vh">
      <Container maxW="container.sm">
        ...
        <form.Form onSubmit={(data) => submit.mutateAsync(data)}>
          <FormLayout>
            <form.Field
...
In the next article, we'll add authentication (Better Auth), and a proper API running on Hono, with a Postgres database.

<Info>
  Saas UI Pro now ships with our new Tanstack Router starter kit, that includes
  a full multi-tenant application, with authentication, tRPC, Drizzle ORM,
  Stripe, feature flags, and a lot more.
  <br />
  <strong>Use code `TANSTACK` to get 20% off!</strong>
  <br />
  <br />
  <Link href="/docs/tanstack-router-starter-kit">View docs</Link>
</Info>
...
- [Saas UI v3 (beta) Documentation](https://beta.saas-ui.dev/docs)
