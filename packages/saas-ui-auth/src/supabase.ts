 ...
 *
 * Credits to RedwoodJS for the original implementation.
 */
import type {
  SupabaseClient,
  AuthResponse,
...
import type { DefaultUser } from './provider'
import { createAuthIntegration } from './create-auth'
...
export function createAuth(supabaseClient: SupabaseClient) {
  const authImplementation = createAuthService(supabaseClient)

  return createAuthIntegration(authImplementation)
}

function createAuthService(supabaseClient: SupabaseClient) {
  return {
    type: 'supabase',
    ...
    /**
     * Restore Saas UI authentication state when an OAuth or magiclink
