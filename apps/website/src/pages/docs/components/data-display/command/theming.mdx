...
The `Command` component is a multipart component. The styling needs to be
...
```jsx live=false
import { commandAnatomy } from '@saas-ui/theme/anatomy'
import { createMultiStyleConfigHelpers } from '@chakra-ui/react'

const { definePartsStyle, defineMultiStyleConfig } =
  createMultiStyleConfigHelpers(commandAnatomy.keys)
```
...
export const commandTheme = defineMultiStyleConfig({ baseStyle })
```
...
import { commandAnatomy } from './components/command'

export const theme = extendTheme({
  components: { SuiCommand: commandAnatomy },
})
```
...
export const commandTheme = defineMultiStyleConfig({
  variants: { custom },
})

// Now we can use the new `custom` variant
<Command variant="custom"> ... </Command>
```
