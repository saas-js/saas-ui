...
```jsx live=false
import { appShellAnatomy } from '@saas-ui/theme/anatomy'
...
export const appShellTheme = defineMultiStyleConfig({ baseStyle })
```
...
import { appShellAnatomy } from './components/app-shell'

export const theme = extendTheme({
  components: { SuiAppShell: appShellAnatomy },
})
```
