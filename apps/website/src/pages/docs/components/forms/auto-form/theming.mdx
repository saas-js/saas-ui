...
The `AutoForm` component is a multipart component. The styling needs to be
...
```jsx live=false
import { autoFormAnatomy } from '@saas-ui/theme/anatomy'
import { createMultiStyleConfigHelpers } from '@chakra-ui/react'

const { definePartsStyle, defineMultiStyleConfig } =
  createMultiStyleConfigHelpers(autoFormAnatomy.keys)
```
...
export const autoFormTheme = defineMultiStyleConfig({ baseStyle })
```
...
import { autoFormAnatomy } from './components/auto-form'

export const theme = extendTheme({
  components: { SuiAutoForm: autoFormAnatomy },
})
```
...
export const autoFormTheme = defineMultiStyleConfig({
  variants: { custom },
})

// Now we can use the new `custom` variant
<AutoForm variant="custom"> ... </AutoForm>
```
