...
9. Pro: `npx @saas-ui/cli@rc login` then `add` only items that exist in
   the registry. If `add` 404s or 500s (DataGrid, Filters, Aside,
   SplitPage, Resizer, Toolbar are not public registry items), keep the
   existing `@saas-ui-pro/react` import and record it. Do not invent
   replacements or `file:` Pro links unless the user asks.
