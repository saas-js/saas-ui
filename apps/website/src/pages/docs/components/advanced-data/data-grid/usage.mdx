...
`DataGrid` supports both left and right pinned columns.

To pin a column, use the `columnPinning` prop in `intialState` or `state` props.
Setting it in `state` prop will allow you to change the pinned columns dynamically using React state.
