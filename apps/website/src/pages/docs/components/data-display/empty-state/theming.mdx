...
export const emptyStateTheme = defineMultiStyleConfig({ baseStyle })
```
...
import { emptyStateAnatomy } from './components/empty-state'

export const theme = extendTheme({
  components: { SuiEmptyState: emptyStateAnatomy },
})
```
...
<EmptyState variant="custom"> ... </EmptyState>
```
