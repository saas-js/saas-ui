...
The `FeatureFlags` component is a multipart component. The styling needs to be
...
```jsx live=false
import { featureFlagsAnatomy } from '@saas-ui/theme/anatomy'
import { createMultiStyleConfigHelpers } from '@chakra-ui/react'

const { definePartsStyle, defineMultiStyleConfig } =
  createMultiStyleConfigHelpers(featureFlagsAnatomy.keys)
```
...
export const featureFlagsTheme = defineMultiStyleConfig({ baseStyle })
```
...
import { featureFlagsAnatomy } from './components/feature-flags'

export const theme = extendTheme({
  components: { SuiFeatureFlags: featureFlagsAnatomy },
})
```
...
<FeatureFlags size="xl"> ... </FeatureFlags>
```
...
export const featureFlagsTheme = defineMultiStyleConfig({
  variants: { custom },
})

// Now we can use the new `custom` variant
<FeatureFlags variant="custom"> ... </FeatureFlags>
```
